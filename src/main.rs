...
fn main() {
    ...
    for i in 0..200  {
        for j in  0..200  {
            particles.push(Particle::new(2.5+5.0*f64::cos(f64::from(i)*2.0*pi/f)*f64::sin(f64::from(j)*1.0*pi/f), 2.5+5.0*f64::sin(f64::from(i)*2.0*pi/f)*f64::sin(f64::from(j)*1.0*pi/f), 3.0+5.0*f64::cos(f64::from(j)*1.0*pi/f), 1.0));
            particles.push(Particle::new(-2.5+2.5*f64::cos(f64::from(i)*2.0*pi/f)*f64::sin(f64::from(j)*1.0*pi/f), -2.5+2.5*f64::sin(f64::from(i)*2.0*pi/f)*f64::sin(f64::from(j)*1.0*pi/f), -3.0+2.5*f64::cos(f64::from(j)*1.0*pi/f), 1.0));
        }
    }
    let now = Instant::now();
